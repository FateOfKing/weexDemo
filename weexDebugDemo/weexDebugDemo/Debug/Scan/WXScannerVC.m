...
- (void)openURL:(NSString*)URL
{
    ...
    /*打开注释写入自己的vc
    DBWXBaseViewController * controller = [[DBWXBaseViewController alloc] init];
    controller.url = url;
...
- (BOOL)remoteDebug:(NSURL *)url
{
    ...
    for (NSString *param in [query componentsSeparatedByString:@"&"]) {
        ...
        if ([[elts firstObject] isEqualToString:@"_wx_debug"]) {
            ...
            if ([[[self.navigationController viewControllers] objectAtIndex:0] isKindOfClass:NSClassFromString(@"DBWXBaseViewController")]) {
//                DBWXBaseViewController * vc = (DBWXBaseViewController*)[[self.navigationController viewControllers] objectAtIndex:0];
